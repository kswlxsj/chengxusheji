...
- 独立登录、注册、标题主页、游戏、统一存档管理、结局达成过渡页、视频结局页及 Options 页面；五类结局均先播放各自 OP（如有），再展示带淡入淡出的结局达成页，最后进入旧视频结局页操作读取存档或返回主页；存档管理页会按入口提供主页读取/删除、游戏内存档/读取/删除或新游戏选槽；Options 以页内标签提供四类账号级音量设置、游戏设置（自动存档与可配置快捷键）和五类结局收藏。
...
- 底部常驻物品快捷栏：渲染持有物品的图标与名称（含数字快捷键），点击物品运行其 `inspectEvent`；普通物品会在压暗的完整游戏画面中放大展示图片、名称与说明。手机和手电筒在 2 号车厢尚未照明时经项目 `useLight` 自定义动作优先提供“照亮”选择；饮料可确认饮用以回复 2 点 SAN 并变为空易拉罐，但在未照明的 2 号车厢饮用会因开罐声引来攻击、SAN 归零并进入患者结局；在 Clicker 面前饮用则会直接触发困难战斗；空易拉罐和玻璃瓶均可投掷引开 Clicker。开启光源后保留黑幕，并显示从画面右侧中点指向鼠标的射线状光束。
...
- 页面音乐与游戏内背景音分离：标题页、设置页和结束页由 `src/bgm.js` 管理；游戏页按 `scenes.json` 的场景绑定选择唯一背景音，列车声与2号、7号、被啃食6号、伪4号和花海等专属环境音切换时交叉淡化，同一音轨跨场景不重启。玩家可分别调整页面音乐、游戏背景音、游戏音效和点击音效；点击音效统一覆盖游戏内按钮与各页面的按钮、链接。滑杆 60% 对应原始设计音量，最终输出按“资源默认值 × 剧情单次倍率 ×（滑杆值 ÷ 60%）”计算并限制在浏览器音量上限内。
...
保留完整目录。Windows 10/11 用户直接双击 `启动游戏.cmd`，启动器会用系统自带组件建立仅本机可访问的静态服务器并打开默认浏览器，不需要安装或配置其他软件。首次进入时先注册本地账号，再使用相同的用户名和密码登录。进入标题主页后可以开始游戏；点击对话框或场景空白处推进普通对话，按 `Esc` 或点击“暂停”打开菜单。

第一次运行或遇到启动问题时，请阅读独立的[游戏启动说明](启动说明.md)。

macOS、Linux 或开发环境也可以在 `Game` 目录手动启动静态服务器，例如：

```powershell
python -m http.server 8000 --bind 127.0.0.1
```

然后始终通过 `http://127.0.0.1:8000/` 访问；Windows 启动器默认使用 `http://127.0.0.1:32145/`。更换协议、主机或端口会进入另一份浏览器存储空间。

> [!NOTE]
> 本地账号、正式存档、账号音量设置与结局收藏使用同源 `localStorage`，登录状态与跨页临时数据使用 `sessionStorage`。游戏页也会在当前标签页暂存最近稳定检查点（含待续跑事件游标），因此刷新可从事件边界或原选项恢复；关闭标签页后该暂存失效，仍需读取正式存档。浏览器对 `file:` 地址下存储的行为没有统一保证，因此应双击启动器，而不是直接双击 `index.html`。
...
```text
编译完成：15 个场景，280 个事件，11 个物品，4 个属性，1 个技能，6 个小游戏，43 个音效。
...
| `player-profile.js` | 管理当前账号的四类音量倍率、自动存档开关、五类结局收藏与结局展示目录。 |
...
| `save-manager.js` | 统一渲染三个槽位：主页入口读取/删除，游戏内入口存档/读取/删除，新游戏入口选择槽位。 |
| `main.js` | 游戏页组装入口：新游戏、检查点读取/续跑、事件中暂停保存与终局处理；五类结局均在专属 OP（如有）完成后跳转结局达成过渡页，再进入视频结局页；渲染 HUD 与底部物品快捷栏；自动存档开启时，首次进入新车厢后保存当前槽位；小游戏进行中屏蔽系统暂停（`ui.minigame.isOpen()` 守卫暂停按钮与 Esc）。 |
