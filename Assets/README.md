...
`Assets/` 是美术源素材库，也是 `Game/assets/` 的字节级超集。正式游戏只引用 `Game/assets/`；每个运行素材都必须在 `Assets/` 的相同相对路径存在一份 SHA-256 完全相同的副本。`Assets/` 可以额外保留新版候选和尚未接入游戏的素材。
...
- 不同格式的有效版本可以同名主体、保留不同扩展名，例如 `carriage-07.jpg` 与 `carriage-07.png`。
- 移动或改名后必须同步更新游戏引用、重新生成编译数据，并验证 `Game/assets/` 是 `Assets/` 的同路径同字节子集。
