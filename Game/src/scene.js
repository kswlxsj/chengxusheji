  ...
  class SceneManager {
    ...
    buildRenderSignature(scene) {
      ...
      return JSON.stringify([
        ...
        isSceneHalfDark(scene.id, this.state.flags),
    ...
    render(scene) {
      ...
      const halfDark = isSceneHalfDark(scene.id, this.state.flags);
      ...
      if (halfDark) {
        const halfDarkOverlay = document.createElement("div");
    ...
    handlePointerMove(event) {
      ...
      const targetButton = target && target.closest ? target.closest(".scene-object-hit") : null;
      const targetEntry = targetButton ? this.findCanvasEntry(targetButton) : null;
      const canvasEntry = this.topCanvasEntryAt(event) || targetEntry;
    ...
    const aim = target || { x: width * .52, y: height / 2 };
      ...
      const phone = this.state.flags.light_type === "phone";
      const baseAngle = phone ? 17 : 12;

      context.globalCompositeOperation = "destination-out";
      const drawCone = (degrees, opacity) => {
        const halfWidth = Math.tan(degrees * Math.PI / 180) * length;
        const gradient = context.createLinearGradient(origin.x, origin.y, end.x, end.y);
        gradient.addColorStop(0, `rgba(0, 0, 0, ${opacity})`);
        gradient.addColorStop(.68, `rgba(0, 0, 0, ${opacity * .96})`);
        gradient.addColorStop(1, `rgba(0, 0, 0, ${opacity * .45})`);
    ...
    handleCanvasClick(event) {
      ...
      const regularButton = target && target.closest ? target.closest(".scene-object") : null;
      ...
      const targetEntry = button ? this.findCanvasEntry(button) : null;
      ...
      const entry = canvasEntry && (!rectEntry || canvasZ >= rectZ)
        ? canvasEntry
        : rectEntry || targetEntry;
