...
- 独立登录、注册、标题主页、游戏、统一存档管理、结局达成过渡页、视频结局页及 Options 页面；五类结局均先播放各自 OP（如有），再展示带淡入淡出的结局达成页，最后进入旧视频结局页操作读取存档或返回主页；存档管理页会按入口提供主页读取/删除、游戏内存档/读取/删除或新游戏选槽；Options 以页内标签提供三类账号级音量设置和五类结局收藏。
