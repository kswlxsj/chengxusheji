...
检定结果统一由事件 JSON 的 `outcomes` 承接：骰子函数只计算结果和修改数值，不直接播放剧情。当前平衡基准下，常规 8 点属性的标准检定成功率约 66.7%；3号教育+洞察综合检定阈值为 20（8+8 时成功率 50%）；4号交涉小游戏按答对 0—3 题提供 40% / 55% / 70% / 85% 的最终成功率。SAN 在检定中归零时会直接进入终止流程，不继续播放检定结果事件。
...
```text
编译完成：15 个场景，232 个事件，9 个物品，4 个属性，1 个技能，6 个小游戏，30 个音效。
