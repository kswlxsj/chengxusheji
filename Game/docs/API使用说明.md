...
| `isSceneUnlit(sceneId, flags)` | 场景是否处于无光状态：`carriage_02` 看 `light_used`（首次照明前只留极弱轮廓），`carriage_03` 看 `carriage_03_blackout`（认知崩塌「灯灭了」后淡入黑场，离开3号时由 `E_501` 清除），其余场景恒 `false`。只读判定，供 `SceneManager` 渲染与测试使用。 |
