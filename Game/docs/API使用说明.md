...
- skill 不设计数值、不自行选定既有素材：技能检定、%修正、对抗、素材坐标等一律记入清单（技术细节在执行台账标注类别），正文生成易懂提问交由人工决策。仅当素材区使用者勾选「委托占位补位」时，允许 agent 在 `assets/` 新增简单占位 SVG（自行命名，新增文件经闸门 2 diff 确认，正式素材到位后替换）。
