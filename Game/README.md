...
- 乘务员按 `crew_04_dead`（是否死亡，由 3 号车厢黑包流程的第二次急救失败置位）分支：死亡线走 `E_507` 疯狂低语，含 5 秒红底黑字“停下来”惊吓，超高速输出并震动，可暂停、取消，不扣SAN；在世线走 `E_508`。窗边插话 `E_516` 同样按死亡线播「她不是已经死了吗」。
...
| `tools/test-main-route.mjs` | 主线接线回归：4号车厢首次发现与急救询问、4号折返描写、3号→2号点门驱动与里世界返程接线、Clicker 与控制杆接线。 |
