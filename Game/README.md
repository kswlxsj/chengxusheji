...
> [!NOTE]
> 本地账号、正式存档、账号音量设置、结局收藏与成就使用同源 `localStorage`，登录状态与跨页临时数据使用 `sessionStorage`。游戏页也会在当前标签页暂存最近稳定检查点（含待续跑事件游标），因此刷新可从事件边界或原选项恢复；关闭标签页后该暂存失效，仍需读取正式存档。浏览器对 `file:` 地址下存储的行为没有统一保证，因此应双击启动器，而不是直接双击 `index.html`。
...
| `player-profile.js` | 管理当前账号的四类音量倍率、自动存档开关、六类结局收藏、九项成就与展示目录。 |
...
| `settings.js` | 管理 Options 页标签、音量滑杆与自动存档开关的即时保存，以及结局和成就收藏卡渲染。 |
| `login.js` / `register.js` | 处理登录、注册表单和注册后用户名预填。 |
| `save-manager.js` | 统一渲染三个槽位：主页入口读取/删除，游戏内入口存档/读取/删除，新游戏入口选择槽位。 |
| `main.js` | 游戏页组装入口：新游戏、检查点读取/续跑、事件中暂停保存与终局处理；六类结局均在专属 OP（如有）完成后跳转结局达成过渡页，再进入视频结局页；渲染 HUD 与底部物品快捷栏；自动存档开启时，首次进入新车厢后保存当前槽位；小游戏进行中屏蔽系统暂停（`ui.minigame.isOpen()` 守卫暂停按钮与 Esc）。 |
