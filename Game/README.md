...
**交互方式与主分支一致**：前进 / 回头 / 深处等**位移一律点击车厢门热点**触发（`assets/door.svg`，与主线各车厢门同一套左右布局约定），窗外、玻璃瓶等可交互物用 **StillLife 整幅蒙版贴图**（`fullCanvas: true`，按不透明像素命中 + 悬停高亮）。**例外：花海·车门外不设门热点，去留由 `E_510` 的选择菜单决定**（`继续深入` / `调头`）。各场景的热点：
...
E_501 进入里世界（空车厢）→ E_502 空车厢（点右门前进／点左门试图回头）
  ...
  → E_509（点右门继续深入 → E_510 花海 → 选择：继续深入 → E_511 结局「迷失」
          点左门原路返回 → E_503（循环）        调头 → E_513 → 原路返回 →
...
其中 `E_503_BACK` / `E_509_BACK` 是两个"门路由"事件：同一扇左门在"正向探索"与"反向行走（已调头）"两种状态下指向不同去处（反向时分别前往 `E_522` 鬼打墙与 `E_514` 伪2号车厢），由旗标 `ev_inner_backtrack` / `ev_fake04_from_sea` 区分。**花海是循环的**：`E_513` 的「原路返回」经 `E_509_RESUME` 清掉"自花海而来"标记后回到 `E_509` 岔路点，玩家可再次点右门回花海、或从伪4车厢原路返回。
