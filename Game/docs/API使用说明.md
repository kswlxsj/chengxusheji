...
| `invisible` | 否 | `true` 表示**隐形命中区**：不渲染任何贴图，只用 `position` 矩形接收点击（鼠标移上去仍是手型，键盘也能聚焦）。用于"背景美术上已经画好的东西"——例如里世界车厢门上那扇已经画在背景里的车门，热点就是那块区域本身，没有具体物件。`image` 仍需填写（Schema 要求），运行时不加载。 |
| `noHighlight` | 否 | `true` 时关闭悬停/聚焦高亮（不减亮、不放大），只保留手型光标。用于点击区落在背景美术上、不需要叠加特效的物件；建议与 `invisible` 搭配使用。 |
...
- **点门驱动**：跨车厢只能由玩家点门完成，剧情事件不得用 `changeScene` 替玩家进车（否则玩家没有机会在门前做选择）。例：`door_03_to_02` → `E_501` 同时是里世界入口，3号车厢剧情（`E_022_ITEM`）拿完手电筒后必须停下、等玩家点这扇门。
...
- **里世界静音**：进入 `carriage_inner_01`、`carriage_inner_02`、`carriage_fake_04`、`flower_sea`、`flower_sea_inside` 时，游戏页暂停列车背景音，并只放行 `door_open`、`door_locked`、`fake` 与检定三音（`dice_rolling`、`dice_success`、`dice_fail`，见 `main.js` 的 `INNER_WORLD_ALLOWED_SOUNDS`）；其他事件音均静默。检定音属于玩家主动触发检定的即时反馈而非里世界环境音，因此在静音区内照常播放（例如 `E_504` 的侦察检定）。回到真实车厢后恢复。
