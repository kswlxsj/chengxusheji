...
- 独立登录、注册、标题主页、游戏、统一存档管理、统一结局达成页及 Options 页面；五类结局均先播放各自 OP（如有）再进入结局页，页面以对应代表画面、标题与摘要展示达成记录，并提供读取存档和返回主页；存档管理页会按入口提供主页读取/删除、游戏内存档/读取/删除或新游戏选槽；Options 以页内标签提供三类账号级音量设置和五类结局收藏。
...
| `main.js` | 游戏页组装入口：新游戏、检查点读取/续跑、事件中暂停保存与终局处理；五类结局均在专属 OP（如有）完成后跳转统一结局达成页；渲染 HUD 与底部物品快捷栏；自动存档开启时，首次进入新车厢后保存当前槽位；小游戏进行中屏蔽系统暂停（`ui.minigame.isOpen()` 守卫暂停按钮与 Esc）。 |
