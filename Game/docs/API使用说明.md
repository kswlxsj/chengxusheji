...
| `isSceneUnlit(sceneId, flags)` | 场景是否处于完全无光状态：`carriage_02` 看 `light_used`（首次照明前只留极弱轮廓；照明后退出全黑但仍由定向光束黑幕遮挡），`carriage_03` 看 `carriage_03_blackout`（认知崩塌「灯灭了」后淡入黑场，离开3号时由 `E_501` 清除），其余场景恒 `false`。只读判定，供 `SceneManager` 渲染与测试使用。 |
| `isSceneDirectionallyLit(sceneId, flags)` | 场景是否启用鼠标定向光束；仅在 2 号车厢且 `light_used === true` 时成立。光源起点固定为画面右侧中点，鼠标决定射线方向；手机光束比手电筒略宽。 |
