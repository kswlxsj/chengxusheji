...
E_501 进入里世界（空车厢）→ E_502 空车厢（点右门前进／点左门试图回头）
  · 左门试图回头 = E_502_RETURN：按权重静默掷一次（10% 回被啃食的 6 号车厢 → E_525；
    60% 门被关死 → 转身向前 → E_503；30% 回到 3 号车厢 → E_502_CARRIAGE03）
  → E_503 花草车厢（点瓶拾取、点窗外侦察、点右门前进、点左门回头）
    · 左门回头 = E_503_BACK：正向探索时按权重静默掷一次（50% 折返后仍回到空车厢 → E_502；
      50% 摇头继续前进 → E_503_RETURN_FORWARD → E_505）；已在回程态时直接 → E_523
...
E_525 被啃食的 6 号车厢（E_502 回头掷中 10% 时进入；置 carriage_06_eaten，接主线 E_002）
E_502_CARRIAGE03 回到 3 号车厢（E_502 回头掷中 30% 时进入；可再次尝试推 2 号门）
E_503_RETURN_FORWARD 摇头继续前进（E_503 回头掷中 50% 时进入）
```
...
未接线的部分：`E_507` 的 SAN 损失数值剧本尚未定稿；`E-511`（迷失）与 `E_515`（Trauma，文本与旗标已接入）两个新结局类型尚未注册进结束页；`E-525` 触发后的"6 号车厢永久变红"因缺素材暂缓。详见 `docs/conversion-reviews/review-checklist-2026-09-12-1203.md` 的执行台账。

**随机回头线（`weightedBranch`）**：`E_502` 与 `E_503` 两处「（随机：…）」由框架白名单动作 `weightedBranch`（`custom-actions.js`，见 `docs/API使用说明.md` 自定义动作一节）落地——按权重静默掷一次，把选中项的旗标置 `true`、其余置 `false`，事件再用 `conditionalJump` 分支，**不弹任何窗口**。`E_503` 的左门在回程态（`ev_inner_backtrack`）会跳过随机直接去 `E_523`。
...
```text
编译完成：12 个场景，155 个事件，7 个物品，8 个属性，6 个技能，6 个小游戏。
