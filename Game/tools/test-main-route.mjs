...
const endingPageScriptSource = await read("src/ending-page.js");
const endingRevealScriptSource = await read("src/ending-reveal-page.js");
assert.match(loginSource, /markHomeOpIntent\("login"\)/, "登录成功进入主页前应请求 OP");
assert.match(endingPageScriptSource, /markHomeOpIntent\("ending"\)/, "视频结局页返回主页前应请求 OP");
assert.match(endingPageSource, /ending-video-stage/, "视频结局页应保留视频舞台");
assert.match(endingPageScriptSource, /ending-video-stage[\s\S]*ending-video-ratio/, "视频结局页的热区应与视频舞台共用比例");
assert.match(endingPageScriptSource, /videoWidth[\s\S]*videoHeight[\s\S]*loadedmetadata/, "视频结局页应读取素材原始比例");
...
assert.match(endingRevealScriptSource, /--ending-background-image[\s\S]*navigate\("ending"/, "过渡页应展示结局背景后进入视频结局页");
assert.match(endingRevealScriptSource, /600[\s\S]*3400/, "过渡页应按既定时长自动淡出");
