...
assert.match(endingPageSource, /ending-page-backdrop[\s\S]*ending-page-unlocked/, "结局页应提供统一的达成展示");
assert.match(endingPageSource, /--ending-background-image/, "结局页应按目录中的结局画面设置背景");
assert.doesNotMatch(mainSource, /codex-ending-overlay/, "游戏页不应保留视频透明热区");
assert.match(mainSource, /reason === "true_end"[\s\S]*reason === "fake_end"[\s\S]*reason === "bad_end"[\s\S]*reason === "san"[\s\S]*reason === "lost"[\s\S]*navigateToEnding\(reason\)/, "所有结局 OP 后都应进入统一结局页");
